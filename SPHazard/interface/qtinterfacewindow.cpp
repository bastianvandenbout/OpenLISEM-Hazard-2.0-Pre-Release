...
int QTInterfaceWindow::Create(ParameterManager * pm, SPHModel * m, ScriptManager * sm, WorldWindow * window )
{


    SPH_DEBUG("Creating Interface using QT")
...
void QTInterfaceWindow::ImportRunfile(QString name)
{
    ...
    SPH_STATUS("import runfile " + name);
...
void QTInterfaceWindow::ExportRunfile(bool saveas)
{
    ...
    if (!fileName.isEmpty())
    {
        ...
        if(res != 0)
        {
            SPH_ERROR("could not write to file: " + fileName);
        }
    }
...
bool QTInterfaceWindow::OnFileOpenRequest(QString filepath, int type)
{
    ...
    if(type == SPH_FILE_TYPE_MAP)
    {
        use = true;
        m_MapViewTool->AddLayerFromFile(filepath);
    }else if(type == SPH_FILE_TYPE_UNKNOWN){

        if(filepath.length() > 3)
        {
            QString ext = filepath.right(4);

            if((ext.compare(".map") == 0) || (ext.compare(".tif") == 0) || (ext.compare(".asc") == 0))
            {
