...
#define SPH_MAX_FREQ_CRS 10

extern MatrixTable SPH_CRSTable;

inline static void LoadCRSTable(QString dir)
{
    SPH_CRSTable.LoadFromFileCSV(dir + "/" + "pcs.csv");
}
...
public:
    inline CRSSelectWidget() : QDialog()
    {
        ...
        m_PathFreq = QCoreApplication::applicationDirPath() + "/crsfeq" + SPH_FILE_EXT_SETTING;
        ...
        QString datamodel;
        for(int i = 2 ; i < SPH_CRSTable.GetNumberOfRows(); i++)
        {
            datamodel.append(SPH_CRSTable.GetValueStringQ(i,1) + " \t "  + SPH_CRSTable.GetValueStringQ(i,0) + "\n");
        }
    ...
    inline void UpdateFreqCRSList()
    {
        ...
        if (fin.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text))
        {
            ...
            for(int i = 0; i < std::min(m_CRSFreqList.length(),SPH_MAX_FREQ_CRS); i++)
            {
    ...
    inline void SetCurrentCRSFromCurrentIndex()
    {
        ...
        SPH_ERROR("Could not parse EPSG code: " +data.toString());
