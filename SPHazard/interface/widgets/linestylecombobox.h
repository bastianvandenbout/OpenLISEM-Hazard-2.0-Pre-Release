...
static inline void DrawLineStyleToRect(QPainter * p, QRect r, SPHLineStyle s)
{
    QRect rect = r;
...
public:
    ...
    SPHLineStyle m_CurrentLineStyle;

    QList<LineStyleData> m_List;

    inline LineStyleComboBox(QWidget * p = nullptr) : QComboBox(p)
    {
        m_CurrentLineStyle = GetDefaultLineStyle(SPH_LINESTYLE_BLACK);

        QList<LineStyleData> gdlist;
        for(int i = 0; i < SPH_LINESTYLE_NUM; i++)
        {
             SPHLineStyle grad =GetDefaultLineStyle(i);
    ...
    inline void SetCurrentLineStyle(SPHLineStyle ls)
    {
    ...
    inline SPHLineStyle GetCurrentGradient()
    {
