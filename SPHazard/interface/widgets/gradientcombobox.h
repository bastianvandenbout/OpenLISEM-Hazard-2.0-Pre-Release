...
public:
    ...
    SPHColorGradient * m_Current;
    inline GradientDelegate(QList<GradientData> *list, SPHColorGradient * current) : QItemDelegate()
    {
        m_List = list;
        m_Current = current;
    }
...
public:
    ...
    SPHColorGradient m_CurrentGradient;
    ...
    {
        m_CurrentGradient = GetDefaultGradient(SPH_GRADIENT_UI);
        ...
        {
            SPHColorGradient grad =GetDefaultGradient(0);
            GradientData d = GradientData();
            d.grad = grad;
            this->addItem(QString::number(0),QVariant(0));
        }

        for(int i = 0; i < SPH_GRADIENT_NUM; i++)
        {
            SPHColorGradient grad =GetDefaultGradient(i);
            GradientData d = GradientData();
            d.grad = grad;
            gdlist->append(d);
    ...
    inline void SetCurrentGradient(SPHColorGradient grad)
    {
        grad.MakeValid();
    ...
    inline SPHColorGradient GetCurrentGradient()
    {
...
public slots:
    ...
    inline void OnGradientEdited()
    {
        ...
        {
            m_CurrentGradient = SPHColorGradient(m_GradientDialog->stops,m_GradientDialog->colors);
