...
void GL3DCamera::ResizeViewPort(int w, int h)
{
    ...
    m_viewportSize = SPHVector2(float(w),float(h));
...
void GL3DCamera::SetCurrentMatrix()
{
    ...
    SPHMatrix4x4 matrix1;
    SPHMatrix4x4 matrix2;
    matrix1.SetIdentity();
    matrix2.SetIdentity();
    SPHMatrix4x4 matrix3;
    matrix3.SetIdentity();
    ...
    matrix2.SetLookAt(m_Position,m_Position+m_ViewDir,SPHVector3(0.0,1.0,0.0));
    matrix1.SetLookAt(SPHVector3(0,0,0),m_ViewDir,SPHVector3(0.0,1.0,0.0));
    matrix3.SetLookAt(SPHVector3(0,m_Position.y,0),SPHVector3(0,m_Position.y,0)+m_ViewDir,SPHVector3(0.0,1.0,0.0));
...
void GL3DCamera::RotateX(double Angle)
{
    ...
    m_ViewDir = SPHVector3(m_ViewDir*cos(Angle*PIdiv180) + m_Up*sin(Angle*PIdiv180)).Normalize();
    m_ViewDir = m_ViewDir.Normalize();
    //now compute the new UpVector (by cross product)
    m_Up = SPHVector3(0.0,1.0,0.0);//SPHVector3::CrossProduct(m_ViewDir, m_Right)*-1;

    //m_Right = SPHVector3::CrossProduct(m_ViewDir, m_Up);
    //m_Right.setY(0.0);
...
void GL3DCamera::RotateAroundRight(double Angle)
{
    ...
    //now compute the new UpVector (by cross product)
    m_Up = SPHVector3(0,1,0);//SPHVector3::CrossProduct(m_ViewDir, m_Right)*-1;
...
void GL3DCamera::RotateY(double Angle)
{
    ...
    m_ViewDir =  SPHVector3(m_ViewDir*cos(Angle*PIdiv180) - m_Right*sin(Angle*PIdiv180)).Normalize();
...
void GL3DCamera::RotateZ(double Angle)
{
    ...
    //Rotate viewdir around the right vector:
    m_Right =  SPHVector3(m_Right*cos(Angle*PIdiv180) + m_Up*sin(Angle*PIdiv180)).Normalize();
...
{
    SPHVector3 look= SPHVector3(x,y,z);
...
{
    m_Position = SPHVector3(b.GetCenterX(), b.GetSizeX(),b.GetCenterY());
    m_ViewDir = SPHVector3(0,-0.8,0.2).Normalize();
}
