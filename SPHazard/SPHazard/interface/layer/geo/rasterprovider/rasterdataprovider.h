...
#define SPH_RASTER_SAMPLE_NEAREST 0
    ...
    inline void RasterDataProviderInitFromMemList(QList<QList<cTMap *>> Maps, bool duo = false, bool own = false)
    {
        ...
        for(int i = 0; i < Maps.length(); i++)
        {
            if(i == 0)
            {
                ...
                if(n_bands == 0)
                {
                    SPH_ERROR("error in adding maps: Zero bands in map add request!")
                    m_HasData = false;
                    break;
                }
                if(duo)
                {
                    if(n_bands != 2)
                    {
                        SPH_ERROR("error in adding maps: Duo map does not have 2 bands!")
                        m_HasData = false;
                        break;
                    }
                }
                ...
                for(int j = 0; j < Maps.at(i).length(); j++)
                {
                    ...
                    {
                        SPH_ERROR("error in adding maps: Timeseries are not all of equal raster size!")
                        m_HasData = false;
                        break;
                    }
                }
                ...
                {
                    SPH_ERROR("error in adding maps: Timeseries are not all of equal band count!")
                    m_HasData = false;
                    break;
                }
            }
...
public:
    ...
    inline double GetValueAtLocation(SPHVector2 pos, int mode = SPH_RASTER_SAMPLE_NEAREST, int band = 0, int timeindex = 0)
    {
        ...
        //if(mode == SPH_RASTER_SAMPLE_NEAREST)
        {
            if(m_IsMemoryMap)
            {
    ...
    inline bool Save(QJsonObject * obj)
    {
        if(m_IsSaveAble)
        {
            SPHSerialize::FromQListString(obj,"rasterfiles",m_FilesOrg);
            SPHSerialize::FromBool(obj,"is_duo",m_IsDuo);
            SPHSerialize::FromBool(obj,"is_memory",m_IsMemoryMap);
    ...
    inline QList<double> GetTimeSeries(SPHVector2 loc, int band = 0)
    {
        QList<double> ret;
        ...
        if(m_HasTime)
        {
            for(int i = 0; i < m_Times.length(); i++)
            {
                ret.append(GetValueAtLocation(loc,SPH_RASTER_SAMPLE_NEAREST, band, i));
            }
