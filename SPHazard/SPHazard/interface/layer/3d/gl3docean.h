...
public:
    ...
    inline void OnPrepare(OpenGLCLManager * m,GeoWindowState s) override
    {
        ...
        m_TextureI1->Create2DFromFile(m->m_AssetDir + "abstract.jpg");
        m_TextureI2 = new OpenGLCLTexture();
        m_TextureI2->Create2DFromFile(m->m_AssetDir + "cloudnoise.jpg");

        m_TextureH = new OpenGLCLTexture();
        m_TextureH->Create2DFromFile(m->m_AssetDir + "ocean_waveheight.jpg");
        m_TextureN = new OpenGLCLTexture();
        m_TextureN->Create2DFromFile(m->m_AssetDir + "ocean_normal.png");
        m_GeometryPlane = new GL3DGeometry();
        m_GeometryPlane->CreateFromInfiniteCenteredGrid(50);

        m_ColoredTerrainProgram = m->m_GLProgram_uiocean;//m->GetMGLProgram(m->m_AssetDir + "terrain_vs.glsl", m->m_AssetDir + "terrain_fs.glsl", m->m_AssetDir + "terrain_gs.glsl", m->m_AssetDir + "terrain_tc.glsl", m->m_AssetDir + "terrain_te.glsl");
