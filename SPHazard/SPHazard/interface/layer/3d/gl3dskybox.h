...
public:
    ...
    inline void OnPrepare(OpenGLCLManager * m,GeoWindowState s) override
    {
        ...
        m_TextureH->Create2DFromFile(m->m_AssetDir + "abstract.png");
        m_TextureN = new OpenGLCLTexture();
        m_TextureN->Create2DFromFile(m->m_AssetDir + "cloudnoise.jpg");
        m_GeometryBox = new GL3DGeometry();
        m_GeometryBox->CreateFromUnitBox();

        m_SkyBoxProgram = m->GetMGLProgram(m->m_KernelDir + "skybox.vert", m->m_KernelDir + "skybox.frag");
