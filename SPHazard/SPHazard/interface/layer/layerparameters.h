...
public:
    ...
    static const int SPH_LAYERPARAMETER_TYPE_BOOL = 0; //bool
    static const int SPH_LAYERPARAMETER_TYPE_COUNT = 1; //int
    static const int SPH_LAYERPARAMETER_TYPE_NUMBER = 2; //double
    static const int SPH_LAYERPARAMETER_TYPE_COLOR = 3; //SPHColor (4 x float)
    static const int SPH_LAYERPARAMETER_TYPE_VEC3 = 4; //SPHVec3 (3 x float)
    static const int SPH_LAYERPARAMETER_TYPE_TEXTURE = 5; //QImage
    static const int SPH_LAYERPARAMETER_TYPE_3DOBJECT = 6; //OpenGLCLObject
    static const int SPH_LAYERPARAMETER_TYPE_STRING = 7; //OpenGLCLObject
    ...
    inline void AddParameter(QString name, bool & value, bool defeault = false)
    {
        ...
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_BOOL;
    ...
    inline void AddParameter(QString name, int & value, int defeault = 0, bool range = false, int min = 0, int max = 100, bool slider = false)
    {
        ...
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_COUNT;
    ...
    inline void AddParameter(QString name, double & value, double defeault = 0.0, bool range = false, double min = 0, double max = 100.0, bool slider = false)
    {
        ...
        ParameterValue[name] = QString::number(value);
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_NUMBER;
        ParameterMaxVal[name] = max;
        ParameterMinVal[name] = min;
        ParameterHasLimit[name] = range;
        ParameterHasSlider[name] = slider;
    ...
    inline void AddParameter(QString name, ColorF & value, ColorF defeault = ColorF(1.0,1.0,1.0,1.0))
    {
        ...
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_COLOR;
        ParameterMutex.unlock();
    ...
    inline void AddParameter(QString name, SPHVector3 & value, SPHVector3 defeault = SPHVector3(0.0,1.0,0.0))
    {
        ParameterMutex.lock();
        ParameterNames.append(name);
        ParameterDefaultValues[name] = defeault.toString();
        ParameterValue[name] = value.toString();
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_VEC3;
        ParameterMutex.unlock();
    ...
    inline void AddParameter(QString name, QImage & value, QString defeault = "")
    {
        ...
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_TEXTURE;
        ParameterMutex.unlock();
    ...
    inline void AddParameter(QString name, OpenGLCLObject & value, QString defeault = "")
    {
        ...
        ParameterValue[name] = defeault;
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_3DOBJECT;
        ParameterMutex.unlock();
    ...
    inline void AddParameter(QString name, QString & value, QString defeault = "")
    {
        ...
        ParameterMemory[name] = (void*) (&value);
        ParameterTypes[name] = SPH_LAYERPARAMETER_TYPE_STRING;
        ParameterMutex.unlock();
    ...
    inline void internal_updateparam(QString in_name, QString in_value)
    {
        ...
        if(ParameterTypes.contains(name) && ParameterMemory.contains(name) && ParameterDefaultValues.contains(name))
        {
            ...
            if(type == SPH_LAYERPARAMETER_TYPE_BOOL)
            {
            ...
            }else if(type == SPH_LAYERPARAMETER_TYPE_COUNT)
            {
                ...
                *mem = val_convert;
            }else if(type == SPH_LAYERPARAMETER_TYPE_NUMBER)
            {
                ...
                ParameterValue[name] = QString::number(val_convert);
                *mem = val_convert;
            }else if(type == SPH_LAYERPARAMETER_TYPE_STRING)
            {
            ...
            }else if(type == SPH_LAYERPARAMETER_TYPE_COLOR)
            {
            ...
            }else if(type == SPH_LAYERPARAMETER_TYPE_VEC3)
            {
                SPHVector3 * mem = static_cast<SPHVector3*>(ParameterMemory[name]);
                SPHVector3 val_convert;
                val_convert.fromString(value);
                if(ParameterCallback.contains(name))
                {
                    ParameterCallback[name](value, mem);
            ...
            }else if(type == SPH_LAYERPARAMETER_TYPE_TEXTURE)
            {
            ...
            }else if(SPH_LAYERPARAMETER_TYPE_3DOBJECT)
            {
                QString * mem = static_cast<QString*>(ParameterMemory[name]);
                QString val_convert = value;
                if(ParameterCallback.contains(name))
    ...
    inline void Save(QJsonObject * obj)
    {
        ...
        for(int i = 0; i < ParameterNames.length(); i++)
        {
            ...
            if(ParameterValue.contains(name))
            {
                ...
                SPHSerialize::From(obj,name,value);
