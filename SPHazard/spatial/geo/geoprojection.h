...
public:
    ...
    void           AS_AddRef            ();
    void           AS_ReleaseRef        ();
    GeoProjection* AS_Assign            (GeoProjection*);

};

//Angelscript related functionality

inline void GeoProjection::AS_AddRef()
{
    AS_refcount = AS_refcount + 1;

}

inline void GeoProjection::AS_ReleaseRef()
{
    AS_refcount = AS_refcount - 1;
    if(AS_refcount == 0)
    {
...
inline GeoProjection* GeoProjection::AS_Assign(GeoProjection* sh)
{
    *this = *sh;
    return this;
}
...
inline GeoProjection* AS_GetGeoProjectionFromEPSG(int epsg)
{
    ...
    if(ret->IsGeneric())
    {
        SPHS_STATUS("Could not get CRS for epsg code: " + QString::number(epsg));
    }
    return ret;
}
inline GeoProjection* AS_GetGeoProjectionFromWKT(std::string wkt)
{
    ...
    if(ret->IsGeneric())
    {
        SPHS_STATUS("Could not get CRS for wkt code: " + QString(wkt.c_str()));
    }
    return ret;
}
inline GeoProjection* AS_GetGeoProjectionFromWGS84UTM(int zone, bool north)
{
    ...
    if(ret->IsGeneric())
    {
        SPHS_STATUS("Could not get CRS for utm zone: " + QString::number(zone) + (north? "n" : "s") );
    }
    return ret;
}
