...
public:
    ...
    {
        if(m_Transform != nullptr && m_IsValid != false && !m_IsGeneric)
        {
            try
            {
                try
            ...
            {
                SPH_ERROR("Error when transforming raster coordinates");
            }
        }
    }
    ...
    inline SPHVector2 Transform(SPHVector2 in)
    {
        if(m_Transform != nullptr && m_IsValid != false && !m_IsGeneric)
        {
            double x = in.x;
    ...
    inline BoundingBox Transform(BoundingBox b)
    {
        ...
        SPHVector2 tl = SPHVector2(b.GetMinX(),b.GetMinY());
        SPHVector2 br = SPHVector2(b.GetMaxX(),b.GetMaxY());
