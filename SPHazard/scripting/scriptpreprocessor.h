...
inline static QString PreProcessScript(QString script)
{
    ...
    QStringList exts = {".map",".tif",".asc",".shp",".csv",".tbl",".las"};
    bool shape[7] = {false,false,false,true,false,false,false};
    bool table[7] = {false,false,false,false,true,true,false};
    bool pointcloud[7] = {false,false,false,false,false,false,true};
        ...
        for(int i = 0; i < exts.length();i++)
        {
            ...
            while(loc > -1)
            {
                ...
                if(!is_q[end-2])
                {
                    ...
                    if(!is_shape && !is_table && !is_pointcloud)
                    {
                        if(is_assign)
                        {
