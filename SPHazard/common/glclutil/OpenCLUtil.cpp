...
void SetPlatformPreference(const QString& s, int preferred_id)
{
    ...
    {
        SPH_ERROR("Unknown platform, did not set preference!");
    }
...
void ListPlatforms()
{
    ...
    try {
        ...
        SPH_DEBUG(" ");
        SPH_DEBUG("List all Platforms and Devices");
        ...
        for(PlatformIter it=platforms.begin(); it<platforms.end(); ++it) {
            ...
            SPH_DEBUG("Found Platform " + platformname);
            ...
            for (unsigned d=0; d<devices.size(); ++d) {
                ...
                if (checkExtnAvailability(devices[d],CL_GL_SHARING_EXT)) {
                    ...
                    SPH_DEBUG("Found Device " + devicename + " id: " + QString::number(i) + " computetype " + QString(typestr) + " clversion:" + clversion + " mem (mb):" + QString::number(computemem/1000000) +" compute units:" + QString::number(computeunits) +" clock (Mhz):" + QString::number(computeclock) + " gl/cl interop true");
                }else
                {
                    SPH_DEBUG("Found Device " + devicename + " id: " + QString::number(i)  + " computetype " + QString(typestr) + " clversion:" + clversion + " mem (mb):" + QString::number(computemem/1000000) +" compute units:" + QString::number(computeunits) +" clock (Mhz):" + QString::number(computeclock) + " gl/cl interop false");
                }
                i++;
            }
        ...
        SPH_ERROR(QString(err.what()) + "(" + QString(err.err()) + ")");
        error = err.err();
        SPH_ERROR("Error during search for OpenCL platform");
        throw 1;
    }
    SPH_DEBUG(" ");
    SPH_DEBUG(" ");
...
{
    SPH_DEBUG(QString("Try to find Platform ") +QString(pName.c_str()));
...
Program getProgram(Context pContext, std::string file, cl_int &error)
{
    ...
    try {
        ...
        SPH_ERROR(QString("cl program error ") + err.what() + "(" + err.err() + ")");
        error = err.err();
    }
    return ret_val;
}
