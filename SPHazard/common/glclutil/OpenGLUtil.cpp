...
char* loadFile(const char *fname, GLint &fSize)
{
    ...
    if (file.is_open())
    {
        ...
        file.close();
        SPH_DEBUG(QString("file ") + fname + " loaded");
        return memblock;
    }

    SPH_ERROR(QString("Unable to open file ") +fname);
    throw 5;
    return NULL;
}
...
shaders_t loadShaders(const char * vert_path, const char * frag_path) {
    ...
    if (!compiled)
    {
        SPH_ERROR("Vertex shader not compiled.");
    ...
    if (!compiled)
    {
        SPH_ERROR("Fragment shader not compiled.");
...
shaders_t loadShaders(const char * vert_path, const char * frag_path, const char * geom_path, const char * tc_path, const char * te_path) {
    ...
    if (!compiled)
    {
        SPH_ERROR("Geometry shader not compiled.");
    ...
    if (!compiled)
    {
        SPH_ERROR("Tesselation Control shader not compiled.");
    ...
    if (!compiled)
    {
        SPH_ERROR("Tesselation Evaulation shader not compiled.");
...
void attachAndLinkProgramtess( GLuint program, shaders_t shaders) {
    ...
    if (!linked)
    {
        SPH_ERROR("Program did not link.");
