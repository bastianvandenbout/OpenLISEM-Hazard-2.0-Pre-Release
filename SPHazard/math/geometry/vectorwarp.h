...
inline static ShapeFile * VectorWarp(ShapeFile * s_in,GeoProjection p_out,bool segmentize = false,double dist_max = 1.0, bool simplify = false, bool dist_threshold = 1.0)
{
    if(s_in == nullptr)
    {
        SPHS_ERROR(QString("Could not complete vector warping operation for null object ") + (simplify?" with simplification": "") + (segmentize?" with segmentation": ""));
        throw 1;
    }
...
*/
    ...
    if(out == nullptr)
    {
        SPHS_ERROR(QString("Could not complete vector warping operation") + (simplify?" with simplification": "") + (segmentize?" with segmentation": ""));
        throw 1;
    }
