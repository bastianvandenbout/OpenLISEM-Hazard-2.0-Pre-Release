...
{				
	...
	float sy_zh = max((float)((sy_zh_y1)),(float)((sy_zh_y2)));
        ...
        float Slope = fmaxc((float)(0.0001),sqrt((float)((sx_zh*sx_zh)))+sqrt((float)((sy_zh*sy_zh))));
	...
	float t1 = (forcingup + coh)
        ...
        float t2 = max(0.0001f,(float)(forcing +
                (
                cosa*cosa*(pga/9.81)*(d * sd + 1000.0 *(swh ))
                    +
                    ( d*sd + 1000.0 *(swh ))
                *sina*cosa
                )));
...
{				
	...
	float t1 = (forcingup + coh)
        ...
        float t2 = max(0.0001f,(float)(forcing +
                (
                cosa*cosa*(pga/9.81)*(sde * sd + 1000.0 *(swh ))
                    +
                    ( sde*sd + 1000.0 *(swh ))
                *sina*cosa
                )));
	...
	float fheight = 0;
        float FoScr = (do_initialstability == 1? (1.0/sfc):1.0);
        if(FoS < FoScr && FoS > 0.0)
	{
		...
		float wf = swh/sd;
                ...
                float sfmax = 1.2 / max(0.001f,sfc);
...
//    Cos[ArcTan[(h - h0)/dx]]), h]
                        ...
                        h1 = ((-coh)*h0*sde + h0*sde*sfmax + dx*sde*tanphi -
                              ...
                              sqrt(-4*dx*sde*swh*
                                 tanphi*((-coh)*sde + sde*sfmax - coh*swf*wd + sfmax*swf*wd) +
                                   ...
                                   coh*h0*swf*wd - h0*sfmax*swf*wd - dx*swf*tanphi*wd,2)))/
                                 (2*((-coh)*sde + sde*sfmax - coh*swf*wd + sfmax*swf*wd));
	...
	{	
		...
		float solid_add = min(1.0f,max(0.0f,1.0f-ts)) * frac_fail;
...
kernel
void SlopeStabilityExtended( int dim0,
                                int dim1,
                                float dx,
                                float dt,
                                __read_only image2d_t LOCX,
                                ...
                                )
{
        ...
        float t2 = max(0.0001f,(float)(forcing +
                (
                sina*cosa*(pga/9.81)*(d * sd + 1000.0 *(swh ))
                    +
                    ( d*sd + 1000.0 *(swh ))
                *sina*cosa
                )));
...
kernel
void SlopeFailureExtended( int dim0,
                                int dim1,
                                float dx,
                                float dt,
                                __read_only image2d_t LOCX,
                                ...
                                )
{
        ...
        float t2 = max(0.0001f,(float)(forcing +
                (
                cosa*sina*(pga/9.81)*(sde * sd + 1000.0 *(swh ))
                    +
                    ( sde*sd + 1000.0 *(swh ))
                *sina*cosa
                )));
        ...
        if(FoS < FoScr && FoS > 0.0)
        {
                coh = coh -cosa * sina*(pga/9.81)*(sde *(sd - swh) + 1000.0 *(swh)) * tanphi;
