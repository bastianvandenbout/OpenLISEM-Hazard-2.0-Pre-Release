...
kernel
void _particletomaps(int dim0, 				//horizontal nr of cells
                        ...
                        )
{
	...
	for(int i = 0; i < 9; i++)
	{
        ...
        write_imagef(SH, (int2)(gx,gy), hsn*(dx)*dx*dx);
...
kernel
void _particlefrommaps(int dim0, 			//horizontal nr of cells
			int dim1, 						//vertical nr of cells
			float dx, 						//cell size (meters)
			float dt,						//time step (seconds)
			...
			global float* p_sigma,			//stress tensor of particles (size 4*n_particles, 4 = number of dimensions squared)
                        global float* p_props,			//deviatoric stress tensor of particles (size 4*n_particles, 4 = number of dimensions squared)
                        ...
                        )
{
	...
	for(int i = 0; i < 9; i++)
	{
		...
		float weight = weightfunction(dis,0.5*dx);
            ...
            hfweightedavg = hfweightedavg + weight * (hfn_c);
            ufweightedavg = ufweightedavg + weight * (ufn_c);
            vfweightedavg = vfweightedavg + weight * (vfn_c);
