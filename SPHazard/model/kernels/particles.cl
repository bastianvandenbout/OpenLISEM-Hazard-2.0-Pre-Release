...
float weightfunction(float dis, float dis_ref)
{
...
//within sph discretization, we need a smoothing kernel -> "kernel" meaning a spatial weight function
float weightfunctionnn(float dis, float dis_ref)
{
        //kernel function should be normalized, differentiable, continuous
        //cubic spline kernel by monaghan (1992)
        float dis_norm = dis/dis_ref;
...
//within sph discretization, we need a smoothing kernel -> "kernel" meaning a spatial weight function
float weightfunctionderivative(float dis, float dis_ref)
{
                        ...
                        float dragmult)
{
        ...
        float ShearModulus = 1000000000000.0f;
        float ElasticBulkModulus = 10000000000000.0f;
        ...
        float dilatencyangle = 0.0;
        float sindilatency = sin(dilatencyangle);
        float epsilon0 = 0.5;
        float artificial_power = 3.0;
        ...
        float acc_pfx = 0.0;
        float acc_pfy = 0.0;
        float acc_stressx = 0.0;
        float acc_stressy = 0.0;
        float acc_pcorrx = 0.0;
        float acc_pcorry = 0.0;
        float acc_nviscx = 0.0;
        float acc_nviscy = 0.0;
        float weight_total = 0.0;
        float weight_dtotalx = 0.0;
        float weight_dtotaly = 0.0;
        float weight_dtotalnx = 0.0;
        float weight_dtotalny = 0.0;
        float weight_dtotalxy[2] = {0.0,0.0};
        ...
        for(int i = 0; i < 9; i++)
        {
                ...
                while(in_cell && n_cell < PARTICLE_CELL_MAX && id_iter < i_length)
                {
                        ...
                        if(gx_n == gx_should && gy_n == gy_should)
                        {
                                ...
                                acc_px = acc_px - (1.0/100.0)* (1.0/dx) * (x_n - x) *(1.0f/(distance_rel*distance_rel*distance_rel));
                                acc_py = acc_py - (1.0/100.0)* (1.0/dx) * (y_n - y) *(1.0f/(distance_rel*distance_rel*distance_rel));

                                acc_pfx = acc_pfx - (10.0f/(7.0f*3.14159f*0.25*dx*dx))* (1.0-cohfrac_n)*(9.81 * 10.0/(dx))*area*(x_n - x) *(1.0f/(distance));
                                acc_pfy = acc_pfy - (10.0f/(7.0f*3.14159f*0.25*dx*dx))* (1.0-cohfrac_n)*(9.81 * 10.0/(dx))*area*(y_n - y) *(1.0f/(distance));
                                ...
                                float density_den =max(0.1f*dx,havg)*density;//
                                float densitysq_den = density_den*density_den;

                                float density_den_n = max(0.1f*dx,havg)*density;//
                                ...
                                float alpha_PI = 1.0f;
                                float beta_PI = 1.0f;
                                float c_vel = 600.0;//0.5f *( sqrt(10000000000.0/(max(0.1f,havg*density))) + sqrt(10000000000.0/(max(0.1f,havg_n*density))));// * max(0.0f,min(1.0f,min(p_props[4*index+3],c/100.0f)));
                                ...
                                float Rab[4] = {-0.5*Fn * Rab0,0.0,0.0,-0.5*Fn * Rab1};
                                for(int a = 0; a < 2; a++)
                                {
                                        for(int b = 0; b < 2; b++)
                                        {
                                                ...
                                                weight_dtotalxy[a] = weight_dtotalxy[a] + dwdxy[b]*(dwdxy[b] > 0.0?1.0:-1.0);
                                        }
                                }
        ...
        float stress_I1 = stress[0] + stress[3];//0.5 * (max(0.0f,sign(stress[0])*stress[0]) + max(0.0f,sign(stress[3])*stress[3]));
        ...
        for(int a = 0; a < 2; a++)
        {
                for(int b = 0; b < 2; b++)
                {
                        ...
                        float lambda = (3.0f * alphaifa*ElasticBulkModulus*(epsilon[i_yy1] + epsilon[i_yy2]) + (ShearModulus/sqrt(max(0.01f,stress_J2)))*stressdev[i_ab] * epsilon[i_ab])/(27.0*alphaifa*ElasticBulkModulus *sindilatency + ShearModulus);
        ...
        float stressmax = 100000000000000000000.0;
        ...
        stress_I1 = stress[0] + stress[3];//0.5 * (max(0.0f,sign(stress[0])*stress[0]) + max(0.0f,sign(stress[3])*stress[3]));
        ...
        stress_I1 = stress[0] + stress[3];//0.5 * (min(0.0f,sign(stress[0])*stress[0]) + min(0.0f,sign(stress[3])*stress[3]));
        stress_J2 = 0.5 * (stressdev[0]*stressdev[0] + stressdev[1]*stressdev[2] + stressdev[2]*stressdev[1] + stressdev[3]*stressdev[3]);
        //recalculate deviatoric stress tensor for elastic-plastic material
        for(int a = 0; a < 2; a++)
        {
        ...
        stress_I1 = 0.5 * (min(0.0f,sign(stress[0])*stress[0]) + min(0.0f,sign(stress[3])*stress[3]));
        stress_J2 = 0.5 * (stressdev[0]*stressdev[0] + stressdev[1]*stressdev[2] + stressdev[2]*stressdev[1] + stressdev[3]*stressdev[3]);
        ...
        float amax_stress = 10000000000.0 * dt;
        ...
        float k_act = (1.0- ifasin)/(1.0+ifasin);
        float k_pass = (1.0+ifasin)/(1.0-ifasin);
        ...
        un = un- sf*(1.0-gamma)*(k_act)*(1.0f/fac)*9.81f* outside*dt *(((dhdx)));
        vn = vn- sf*(1.0-gamma)*(k_act)*(1.0f/fac)*9.81f* outside*dt *(((dhdy)));
        ...
        un = un-1.0 *(1.0f/fac)*9.81f* outside*dt *(((sz_x)));
        vn = vn-1.0 *(1.0f/fac)*9.81f* outside*dt *(((sz_y)));
        ...
        float v_balance = ff * fluidvavg  + sf * vn;
        vn = v_balance + (vn - v_balance) * exp(-lfacv);
        float u_balance = ff * fluiduavg  + sf * un;
        ...
        vn = vn > 0? max(0.0f,(float)(vn - sf * dt *(1.0f/fac)* 9.81f * tan(ifa))) : min(0.0f,(float)(vn + sf * dt *(1.0f/fac)* 9.81f * tan(ifa)));
        un = un > 0? max(0.0f,(float)(un - sf * dt *(1.0f/fac)* 9.81f * tan(ifa))) : min(0.0f,(float)(un + sf * dt * (1.0f/fac)*9.81f * tan(ifa)));
        ...
        reportf =(!isnormal(stress_J2))?0.0:log(stress_J2 * (stress_J2 < 0? -1.0f:1.0f))/60;
        ...
        p_ui[2*index+1] = isnan(test)? 10000.0:0.0;
}


//main kernel for particle dynamics (here kernel indicates a gpu-compiled code functions)s
kernel
void _particledynamics2(	__global float* p_xy,                                   //location of particles (size 2*n_particles)
                                __global float* p_uv,			//velocity of particles (size 2*n_particles)
                                __global float* p_props,		//deviatoric stress tensor of particles (size 4*n_particles, 4 = number of dimensions squared)
                                __global float* p_sigma,		//stress tensor of particles (size 4*n_particles, 4 = number of dimensions squared)
                                __global int* p_index,			//index to particles sorted by cell in which they lie
...
)
{
        ...
        p_props[4*index+0] = hsn*(dx)*dx*dx;
