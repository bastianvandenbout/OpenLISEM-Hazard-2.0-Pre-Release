...
{			   
	...
	}else
	{
		...
		float den = pow((UF_Aspect * UT*( P * F + (1.0f-P) * G) + (P/sf_dev + (1.0f-P)/ff_dev) * 10.0f),UF_j);
                float dc = dragmult * sf;//den > 0.0f? 100.0*ff_dev * sf_dev* (1.0f - (1.0f/max(1.0f,density)))/den : 0.0f;
		float aspect = 1.0;
                           ...
                           float ifa, float rocksize, float density, float dragmult)
{
	...
	double acc_x =(-GRAV * sxs_zh + UF_Aspect*pbs*( dhdx)
	...
	float vsq = sqrt((float)(us * us + vs * vs));
        float nsq1 = 0.1 * (0.001+n)*(0.001+n)*GRAV/max(0.01f,pow((float)(hn),(float)(4.0/3.0)));
    float nsq = nsq1*vsq*dt;			
	vn = (float)((vn/(1.0f+nsq)));
                           ...
                           float ifa, float rocksize, float density, float dragmult)
{
	...
	double acc_y =(-GRAV * sys_zh + UF_Aspect*pbs*( dhdy)
	...
	float infilpot = (dt/dt_hydro)*read_imagef(InfilPot,sampler, (int2)(gx,gy)).x;
   	float infilcum = read_imagef(InfilCum,sampler, (int2)(gx,gy)).x;
        ...
        if(z > -1000)
        {
            ...
            float3 hll_x1 = z_x1 < -1000? (float3)(0.0,0.0,0.0):F_HLL2FS(h_x1,hs_x1,vx_x1,vy_x1,h,hs,vx,vy);
            float3 hll_x2 = z_x2 < -1000? (float3)(0.0,0.0,0.0):F_HLL2FS(h,hs,vx,vy,h_x2,hs_x2,vx_x2,vy_x2);
            float3 hll_y1 = z_y1 < -1000? (float3)(0.0,0.0,0.0):F_HLL2FS(h_y1,hs_y1,vy_y1,vx_y1,h,hs,vy,vx);
            float3 hll_y2 = z_y2 < -1000? (float3)(0.0,0.0,0.0):F_HLL2FS(h,hs,vy,vx,h_y2,hs_y2,vy_y2,vx_y2);
