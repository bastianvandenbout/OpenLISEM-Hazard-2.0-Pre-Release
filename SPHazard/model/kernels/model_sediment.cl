...
float GetTC(float hf, float v, float d50, float d90, float slope)
{
                                ...
                                )
{
    ...
    if(do_erosion)
    {
        ...
        float SplashDelivery = 0.1;

        float KEParamater_a = 8.950;
        float KEParamater_b = 0.520;
        float KEParamater_c = 0.042;

        float Int = rain_m * 3600.0/dt * 1000.0;
        // intensity in mm/h, Rain is in m

        double KE_DT = KEParamater_a*(1.0-(KEParamater_b*exp(-KEParamater_c*Int)));
       ...
       //case KE_POWERFUNCTION: KE_DT = KEParamater_a3*pow(Int, KEParamater_b3); break;
        ...
        float directrain = (1.0 -vegcover)*rain_m * 1000;
        // rainfall between plants in mm

        float KE_LD = max(15.3*sqrt(vegh)-5.87, 0.0);
        // kin energy in J/m2/mm
        float throughfall = vegcover * rain_m * 1000.0;

        float WH0 = exp(-1.48*hf*1000.0);
        // water buffer effect on surface, WH in mm in this empirical equation from Torri ?

        strength = 0.1033/max(0.001f,Coh);
        b = 3.58;
        ...
        //deposite anything above MAXCONC
        {
            ...
            deposition = min(deposition <0.0? -deposition:deposition,((minTC * watervol)<0.0f?-1.0f:1.0f)*(minTC * watervol));
        ...
        sflux_x1 = isnan(sflux_x1)?0.0:sflux_x1;
        sflux_x2 = isnan(sflux_x2)?0.0:sflux_x2;
        sflux_y1 = isnan(sflux_y1)?0.0:sflux_y1;
        sflux_y2 = isnan(sflux_y2)?0.0:sflux_y2;

        float qfsout = 0.0f;
        qfsout = qfsout +z_x1 < -1000? sflux_x1 : 0.0;
        qfsout = qfsout +z_x2 < -1000? sflux_x2 : 0.0;
        qfsout = qfsout +z_y1 < -1000? sflux_y1 : 0.0;
        qfsout = qfsout +z_y2 < -1000? sflux_y2 : 0.0;
        qsedout += qfsout;
        ...
        if(chmask > 0 && chmask < 10)
        {
            ...
            float ch_detachment = 0.0;
            float ch_deposition = 0.0;
            ...
            ch_deposition = min(ch_deposition <0.0? -deposition:deposition,((minTC * watervol)<0.0f?-1.0f:1.0f)*(minTC * watervol));
   ...
   //////
    ...
    float UF_ENTRAINMENTCCONSTANT = 1.0;
    float UF_ENTRAINMENTCONSTANT = 0.5;
    ...
    if(false)//materialdepth > 0.001f)
    {
        ...
        float pbs = (1.0-gamma)*(-9.81 * (hf+hs));
        ...
        float sadd = entrainment * 0.6;
        float fadd = entrainment * 0.4;
        ...
        hs = hs + entrainment * 0.6;
        hf = hf + entrainment * 0.4;
    }
